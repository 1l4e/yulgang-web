import {  findManyVHoangShop } from "@/models/shop.server"
...
const VoHoangShop = async ({params,searchParams}) => {
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-3xl text-center py-4">Võ Hoàng Shop</h1>
      <div className="py-8">
        <Table>
          ...
          <TableBody>
              ...
              {data?.map((item:any,index:number)=> (
                 <TableRow key={index} className="text-sm md:text-md">
                    <TableCell>!shopvohoang {item.ID}</TableCell>
