import {  findManyEventShop } from "@/models/shop.server"
...
const EventShop = async ({params,searchParams}) => {
    let {page} = searchParams;
    if (!page || +page <0) page =0;

    if (Number.isNaN(+page)) page = 0

    const data = await findManyEventShop(50,page)
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-3xl text-center py-4">Event Shop</h1>
      <div className="py-8">
        <Table>
          ...
          <TableBody>
              ...
              {data?.map((item:any,index:number)=> (
                 <TableRow key={index} className="text-sm md:text-md">
                    <TableCell>!shopevent {item.ID}</TableCell>
